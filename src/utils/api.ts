 ...
 */
export async function checkBanildToolsAvailability(): Promise<{
  available: boolean;
  version?: string;
  message: string;
}> {
  const url = `${config.url}/wp-json/banildtools/v1/status`;
  try {
    ...
    if (response.ok) {
      ...
      return {
        ...
        message: `BanildTools v${data.version} detected`,
      };
    }
    // 404 means plugin not installed/active
 ...
 */
export async function callBanildToolsAPI(
  endpoint: string,
  method: string = "POST",
  body?: object
): Promise<any> {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `BanildTools API error: ${response.status} ${response.statusText} - ${errorText}`
      );
    }
    return await response.json();
  } catch (error: any) {
 ...
 */
export async function searchWordPressOrgPlugins(
  query: string,
  page: number = 1,
  perPage: number = 10
): Promise<any[]> {
  ...
  for (const url of tryUrls) {
    try {
      ...
      if (!res.ok) {
        lastError = new Error(
          `WordPress.org API error: ${res.status} ${res.statusText}`
        );
        continue;
      }
        ...
        return data.map((item: any) => {
          const slug =
            ...
            (typeof item.url === "string"
              ...
              : "");
          const name =
            item.name?.rendered || item.title?.rendered || item.title || slug;
          const description =
            item.short_description?.rendered ||
...
export async function callWooCommerceAPI(
  ...
  const url =
    config.wcKey && config.wcSecret
      ? `${base}${base.includes("?") ? "&" : "?"}consumer_key=${encodeURIComponent(
          config.wcKey
