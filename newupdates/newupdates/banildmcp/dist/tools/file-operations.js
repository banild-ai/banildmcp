...
export function registerFileOperationTools(server) {
    // ========== READ FILE ==========
    server.tool("banildtools_read_file", async (args) => {
        ...
        description: "Read file contents from WordPress server. Supports line offset and limit for partial reads. Returns base64 for images.",
        schema: {
            target_file: "string",
            offset: "number?",
            limit: "number?",
    ...
    // ========== WRITE FILE ==========
    server.tool("banildtools_write_file", async (args) => {
        ...
        description: "Create or overwrite a file on WordPress server. Creates parent directories if needed.",
        schema: {
            file_path: "string",
            contents: "string",
        },
    ...
    server.tool("banildtools_edit_file", async (args) => {
        ...
        description: "Search and replace text in a file. Set replace_all=true to replace all occurrences, otherwise replaces only the first unique match.",
        schema: {
            file_path: "string",
            old_string: "string",
            new_string: "string",
    ...
    // ========== DELETE FILE ==========
    server.tool("banildtools_delete_file", async (args) => {
        ...
        description: "Delete a file or directory from WordPress server. Directories are deleted recursively. Requires delete permission enabled in plugin settings.",
        schema: {
            target_file: "string",
        },
    });
    // ========== LIST DIRECTORY ==========
    server.tool("banildtools_list_dir", async (args) => {
        ...
        description: "List directory contents on WordPress server. Returns files and directories with metadata. Supports ignore patterns.",
        schema: {
    ...
    // ========== SEARCH FILES (GREP) ==========
    server.tool("banildtools_grep", async (args) => {
        const { pattern, path, glob, case_insensitive = false, context_lines = 0, output_mode = "content", max_results = 500, } = args;
        try {
        ...
        description: "Search for regex patterns in files (grep-like). Supports file glob filtering, context lines, and multiple output modes (content, files_with_matches, count).",
        schema: {
            pattern: "string",
            path: "string?",
            glob: "string?",
    ...
    // ========== GLOB FILE SEARCH ==========
    server.tool("banildtools_glob_search", async (args) => {
        ...
        description: "Find files matching a glob pattern on WordPress server. Results sorted by modification time (newest first).",
        schema: {
            glob_pattern: "string",
            target_directory: "string?",
        },
    ...
    // ========== CREATE DIRECTORY ==========
    server.tool("banildtools_mkdir", async (args) => {
        ...
        description: "Create a directory on WordPress server. Creates parent directories by default (recursive=true).",
        schema: {
            path: "string",
            recursive: "boolean?",
        },
    ...
    // ========== RENAME/MOVE ==========
    server.tool("banildtools_rename", async (args) => {
        ...
        description: "Rename or move a file/directory on WordPress server. Creates destination directory if needed.",
        schema: {
            source: "string",
            destination: "string",
        },
    ...
    // ========== COPY ==========
    server.tool("banildtools_copy", async (args) => {
        const { source, destination } = args;
        try {
        ...
        description: "Copy a file or directory on WordPress server. Directories are copied recursively.",
        schema: {
            source: "string",
            destination: "string",
        },
    ...
    // ========== FILE INFO ==========
    server.tool("banildtools_file_info", async (args) => {
        const { path } = args;
        try {
            const result = await callBanildToolsAPI("/info", "POST", { path });
            return Responses.success(result.info, `ℹ️ Info for: ${result.info.path}`);
        }
        catch (error) {
            return Responses.error(`Failed to get file info: ${error.message}`);
        }
    }, {
        description: "Get detailed information about a file or directory on WordPress server (size, permissions, modified date, line count, etc).",
        schema: {
            path: "string",
        },
    });
    // ========== APPEND FILE ==========
    server.tool("banildtools_append_file", async (args) => {
        const { file_path, contents } = args;
        try {
        ...
        description: "Append content to end of file. Use for writing large files in chunks to avoid timeouts.",
        schema: {
            file_path: "string",
            contents: "string",
        },
    ...
    // ========== GET OPTION ==========
    server.tool("banildtools_get_option", async (args) => {
        const { name } = args;
        try {
    ...
    // ========== SET OPTION ==========
    server.tool("banildtools_set_option", async (args) => {
    ...
    // ========== DELETE OPTION ==========
    server.tool("banildtools_delete_option", async (args) => {
        const { name } = args;
        try {
    ...
    // ========== GET TRANSIENT ==========
    server.tool("banildtools_get_transient", async (args) => {
        const { name } = args;
        try {
    ...
    // ========== SET TRANSIENT ==========
    server.tool("banildtools_set_transient", async (args) => {
    ...
    // ========== DELETE TRANSIENT ==========
    server.tool("banildtools_delete_transient", async (args) => {
        const { name } = args;
        try {
    ...
    // ========== DEBUG LOG ==========
    server.tool("banildtools_debug_log", async (args) => {
        const { action = "tail", lines = 100 } = args || {};
        try {
            const result = await callBanildToolsAPI("/debug-log", "POST", { action, lines });
            if (action === "clear") {
                return Responses.success({ cleared: result.cleared }, `🗑️ Debug log ${result.cleared ? "cleared" : "could not be cleared"}`);
            }
            return Responses.success({
                path: result.path,
                exists: result.exists,
    ...
    // ========== PHP LINT ==========
    server.tool("banildtools_php_lint", async (args) => {
    ...
    // ========== CLEAR CACHE ==========
    server.tool("banildtools_clear_cache", async (args) => {
        const { type = "all" } = args || {};
        try {
    ...
    // ========== SERVER INFO ==========
    server.tool("banildtools_server_info", async () => {
        try {
            const result = await callBanildToolsAPI("/server-info", "GET");
            return Responses.success(result, `🖥️ Server: PHP ${result.php?.version || "?"}, WP ${result.wordpress?.version || "?"}`);
        }
        catch (error) {
            return Responses.error(`Failed to get server info: ${error.message}`);
        }
    }, {
        description: "Get comprehensive server information (PHP, WordPress, Database, Disk, etc).",
        schema: {},
    });
    // ========== DATABASE QUERY ==========
    server.tool("banildtools_db_query", async (args) => {
