...
export function registerPageTools(server) {
    server.tool("wordpress_create_page", async (args) => {
    ...
    server.tool("wordpress_update_page", async (args) => {
    ...
    server.tool("wordpress_get_pages", async (args) => {
        const { perPage = 10, page = 1, parent, status = "publish" } = args;
        try {
            const params = { per_page: perPage, page, status };
            if (parent !== undefined)
                params.parent = parent;
            const queryString = buildQueryString(params);
            const pages = await callWordPressAPI(`/pages?${queryString}`);
            return Responses.success({ pages: pages.map(formatPage), count: pages.length }, `📄 Retrieved ${pages.length} pages`);
        }
        catch (error) {
    ...
    server.tool("wordpress_delete_page", async (args) => {
