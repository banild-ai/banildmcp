...
export function registerMediaTools(server) {
    server.tool("wordpress_upload_media", async (args) => {
    ...
    server.tool("wordpress_get_media", async (args) => {
        ...
        try {
            ...
            return Responses.success({ media: media.map(formatMedia), count: media.length }, `🖼️ Retrieved ${media.length} media items`);
        }
        catch (error) {
    ...
    server.tool("wordpress_update_media", async (args) => {
    ...
    server.tool("wordpress_delete_media", async (args) => {
    ...
    server.tool("wordpress_set_featured_image", async (args) => {
        const { postId, mediaId } = args;
        try {
            const post = await callWordPressAPI(`/posts/${postId}`, "PUT", {
                featured_media: mediaId,
            });
            return Responses.success({ postId, mediaId, set: true }, `✅ Set featured image for post ${postId}`);
        }
        catch (error) {
